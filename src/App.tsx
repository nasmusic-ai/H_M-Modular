...
import { 
  Hammer, Menu, X, ChevronRight, ChevronLeft, MapPin, Phone, Facebook, Instagram, ArrowRight
} from 'lucide-react';
          ...
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center gap-8">
              {['Home', 'About', 'Materials', 'Gallery', 'Contact'].map(item => (
                <a key={item} href={`#${item.toLowerCase()}`} className="text-sm font-medium text-stone-300 hover:text-amber-400 transition-colors uppercase tracking-widest">
                  {item}
                </a>
              ))}
            ...
            {/* Mobile Menu Button */}
            <button className="md:hidden text-stone-300 p-2" onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          {isMenuOpen && (
            ...
            >
              <div className="px-4 pt-2 pb-6 space-y-1">
                {['Home', 'About', 'Materials', 'Gallery', 'Contact'].map(item => (
                  <a key={item} href={`#${item.toLowerCase()}`} onClick={() => setIsMenuOpen(false)} className="block px-3 py-4 text-base font-medium text-stone-300 hover:text-amber-400 border-b border-stone-800">
                    {item}
                  </a>
                ))}
                <div className="pt-4">
        ...
        <div className="max-w-7xl mx-auto px-4">
          <div ref={scrollRef} className="flex overflow-x-auto gap-6 scrollbar-hide">
            {galleryImages.map(img => (
              <div key={img.id} className="min-w-[250px] bg-stone-900 rounded-2xl overflow-hidden border border-amber-900/20">
                <img src={img.url} alt={img.title} className="w-full h-64 object-cover" referrerPolicy="no-referrer" />
                <div className="p-4 text-white">
                  <h3 className="font-bold">{img.title}</h3>
                  <p className="text-stone-400 text-sm">{img.location}</p>
                  <span className="text-amber-400 text-xs">{img.type}</span>
                </div>
              </div>
            ))}
          </div>
          <div className="flex justify-between mt-4">
            <button onClick={() => scroll('left')} className="text-amber-400"><ChevronLeft /></button>
            <button onClick={() => scroll('right')} className="text-amber-400"><ChevronRight /></button>
          </div>
        </div>
      </section>
...
<textarea
  rows={4}
  className="w-full bg-stone-950 border border-stone-800 rounded-xl px-4 py-3 text-white focus:border-amber-600 outline-none transition-colors"
  placeholder="Tell us about your dream cabinet..."
></textarea>
...
<footer className="py-12 bg-stone-950 border-t border-stone-900">
  <div className="max-w-7xl mx-auto px-4 text-center">
    <img
      src={LOGO_URL}
      alt="M&N Logo"
      className="h-12 w-auto mx-auto mb-6 opacity-50 grayscale"
      referrerPolicy="no-referrer"
    />
    <p className="text-stone-600 text-sm mb-4">
      © 2026 M&N Modular Cabinets. Website Developed by Jorjan Lanaja.
    </p>
